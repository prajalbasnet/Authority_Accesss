...
const CitizenNavbar = () => {
  ...
  const getKycBadge = () => {
    ...
    if (status === "VERIFIED" || status === "PENDING") {
      return (
        <span
          className={`ml-4 px-3 py-1 rounded-full text-xs font-bold text-white ${badgeColor} cursor-default`}
        >
          {badgeText}
        </span>
      );
  ...
  const handleNotificationClick = (notificationId) => {
    ...
    setNotificationDropdownOpen(false); // Close dropdown after clicking
  };
