...
import { CheckCircle, TrendingUp, Clock, Users, Globe, Shield } from 'lucide-react';
...
const Advantages: React.FC = () => {
  ...
  const stats = [
    { number: 1000, suffix: "+", label: "Complaints Resolved", icon: CheckCircle },
    { number: 6, suffix: "", label: "Authority Categories", icon: Users },
    { number: 24, suffix: "/7", label: "Tracking & Escalation", icon: Clock },
    { number: 99, suffix: "%", label: "Citizen Satisfaction", icon: TrendingUp }
  ];
  ...
  useEffect(() => {
    ...
    statNumbers?.forEach((stat, index) => {
      ...
      gsap.fromTo({ value: 0 }, {
        ...
        onUpdate: function() {
          if (stat) {
            stat.textContent = Math.round(this.targets()[0].value).toString();
          }
        },
        scrollTrigger: {
    ...
    <section ref={sectionRef} className="py-20 bg-gradient-to-br from-gray-50 to-blue-50">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-red-600 mb-4">Platform Advantages</h2>
          <p className="text-xl text-gray-700">
            Transforming public service delivery with transparency and accountability
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div ref={checklistRef} className="space-y-4">
            <h3 className="text-2xl font-bold text-blue-900 mb-6">Why Choose SunneAawaj?</h3>
            {advantages.map((advantage, index) => (
              <div key={index} className="check-item flex items-center gap-4 p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
          ...
          <div ref={statsRef} className="space-y-8">
            <h3 className="text-2xl font-bold text-red-600 mb-6">Platform Impact</h3>
            <div className="grid grid-cols-2 gap-6">
              {stats.map((stat, index) => {
                const IconComponent = stat.icon;
                return (
                  <div key={index} className="stat-card bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                    <div className="flex items-center justify-between mb-4">
                      <IconComponent className={`w-8 h-8 ${index % 2 === 0 ? 'text-red-600' : 'text-blue-900'}`} />
                      <div className="text-right">
                        <div className={`text-3xl font-bold ${index % 2 === 0 ? 'text-red-600' : 'text-blue-900'}`}>
                    ...
                    <div className={`w-full h-1 rounded-full mt-2 ${index % 2 === 0 ? 'bg-red-100' : 'bg-blue-100'}`}>
                      <div className={`h-1 rounded-full w-full ${index % 2 === 0 ? 'bg-red-600' : 'bg-blue-900'}`}></div>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-600">Language Inclusive</span>
                </div>
                <div className="flex items-center gap-3">
        ...
        <div className="mt-16 text-center">
          <blockquote className="text-xl italic text-gray-700 max-w-4xl mx-auto">
            "SunneAawaj bridges the digital divide, ensuring every Nepali citizen's voice is heard and every complaint reaches the right authority for swift resolution."
          </blockquote>
